...
class ProjectRepository(SQLAlchemyRepository[Project]):
    ...
    def _base_query(self) -> Select[tuple[Project]]:
        ...
        return select(Project).options(selectinload(Project.links))
    ...
    async def search(
        ...
        *,
        ...
        stmt = self._base_query().where(Project.workspace_id.in_(select(WorkspaceMembership.workspace_id).where(WorkspaceMembership.user_id == owner_id)))
        if query:
